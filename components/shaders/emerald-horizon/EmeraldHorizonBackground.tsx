"use client";
/**
 * ThreeUI — EmeraldHorizonBackground. Registered source, file sha256
 * d7769f7f8692c025297e48e7f398308946d2c4108976c047e18ee55c98aff655 (verified
 * against the bundle at threeui.com/source-code/emerald-horizon.json).
 *
 * Two adaptations, both forced by this project's runtime rather than taste:
 *
 *  1. `three128` -> `three`. The bundle imports a pinned alias. This project
 *     already ships three for the contact globe; adding a second copy would
 *     duplicate a ~1.9MB dependency for one background.
 *
 *  2. `outputColorSpace = LinearSRGBColorSpace`. The source targets r128–r160,
 *     where the renderer defaulted to LinearEncoding and wrote gl_FragColor
 *     straight out. This project is on r180, where the default is sRGB, so the
 *     same shader renders visibly brighter and washed out. Forcing linear
 *     restores the authored appearance. Guarded, so it is a no-op on the
 *     versions the source was written for.
 *
 * Everything else — structure, uniforms, the resize and intersection
 * observers, the rAF loop, disposal — is the registered source unchanged.
 *
 * react-hooks/refs is disabled below rather than refactored away: the source
 * writes live prop values into a ref during render so the rAF loop can read
 * them without re-running its effect. Rewriting that is a behaviour change to
 * registered source, which the brief forbids.
 */
...
export function EmeraldHorizonBackground({ className = "", ...props }: EmeraldHorizonBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const scene = new THREE.Scene(); const camera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1); const renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: true }); renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); if ("outputColorSpace" in renderer) renderer.outputColorSpace = THREE.LinearSRGBColorSpace;
    ...
    let frame = 0, visible = true; const start = performance.now();
